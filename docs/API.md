...
Complete reference for every public function in `onebase-js`. For a guided
...
```ts
import { createClient } from "onebase-js"
...
| `auth.storageKey`       | `string`                | `"onebase.auth.session"`    | Key under which the session is stored.              |
...
```ts
const { data, error } = await oc.functions.invoke<{ greeting: string }>(
	"hello",
	{
		body: { name: "world" },
		query: { debug: "1" },
...
arrives without `old`/`new` — you still learn _that_ a change happened.
...
```ts
import { OneCodeError } from "onebase-js"
