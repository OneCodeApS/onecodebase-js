...
export class AuthClient {
	...
	constructor(ctx: ClientContext, opts: AuthClientOptions = {}) {
		...
		this.#storageKey = opts.storageKey ?? "onebase.auth.session"
	...
	async signUp(
		...
		try {
			const payload = await this.#post<AuthApiResponse>(
				"/auth/v1/signup",
				credentials,
			)
	...
	async signInWithOAuth(opts: {
		...
		if (opts.provider !== "microsoft") {
			return {
				data: null,
				error: new OneCodeError(
					`Unsupported OAuth provider: ${opts.provider}`,
				),
			}
		}
	...
	async refreshSession(
		...
		if (!rt) {
			return {
				data: null,
				error: new OneCodeError("No refresh token available", {
					status: 400,
				}),
			}
		}
	...
	#scheduleRefresh(): void {
		...
		const delay = Math.max(
			0,
			this.#session.expires_at - Date.now() - REFRESH_MARGIN_MS,
		)
