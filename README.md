...
Isomorphic JavaScript/TypeScript client for an [onebase](https://github.com/OneCodeApS/onebase) backend — Postgres + PostgREST + auth + storage + realtime + edge functions, behind one `api.<host>` surface. The ergonomics mirror `@supabase/supabase-js`, so if you've used that, this will feel familiar.

```bash
npm install onebase-js
```
